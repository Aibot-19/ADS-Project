...
def main():
    # lists to memorize datas read by CSV
    n_values = []
    bst_times = []
    avl_times = []
    rbt_times = []
    ...
    try:
        with open('misurazioni_alberi.csv', mode='r') as file:
            ...
            next(reader) # skips first line (header w column names)
            
            for row in reader:
                # converts read by CSV strings in numbers (float/int)
    ...
    # sets the size of the figure (inches, proportion suitable for A4)
    plt.figure(figsize=(10, 6))

    # draw the three lines (adds small markers to see the actual points)
    ...
    # 3. customizes axis
    #  this is the key part: we use a logarithmic scale for the X-axis
    #  because we generated n using geometric progression
    plt.xscale('log')
    ...
    # saves graph as a PNG file so it can be inserted into a LaTeX document (for the report)
